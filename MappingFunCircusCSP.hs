...
mapping_CAction (CSPNSParal ns1 cs ns2 a b)
	= mapping_CAction(a)
		...
		++ " |] "
		++ mapping_CAction(b)
mapping_CAction (CSPParAction zn (x:xs))
	= undefined
...
mapping_CAction (CSPRepIntChoice [(Choose x s)] a)
	= "|~| "
		...
		++ " @ "
		++ mapping_CAction(a)
mapping_CAction (CSPRepInterl (x:xs) ca)
	= undefined
...
mapping_CProc (CRepIntChProc [(Choose x s)] a)
	= "|~| "
		...
		++ " @ "
		++ mapping_CProc(a)
mapping_CProc (CRepInterlProc (x:xs) cp)
	= undefined
mapping_CProc (CRepParalProc cse (x:xs) cp)
	= undefined
...
mapping_CProc (ProcMain zp (x:xs) ca)
	= undefined
mapping_CProc (ProcStalessMain (x:xs) ca)
	= undefined
...
mapping_CDecl (CChan zn1)
	= undefined
mapping_CDecl (CChanDecl zn2 ze)
	= undefined
...
mapping_Comm (ChanComm zn (x:xs))
	= undefined
mapping_Comm (ChanGenComm zn (x:xs) (y:ys))
	= undefined
...
mapping_CSExp (ChanSetDiff a b)
	= "diff("++mapping_CSExp(a)++","++mapping_CSExp(b)++")"
mapping_CSExp (ChanSetInter a b)
	= "inter("++mapping_CSExp(a)++","++mapping_CSExp(b)++")"
mapping_CSExp (ChanSetUnion a b)
	= "union("++mapping_CSExp(a)++","++mapping_CSExp(b)++")"
...
mapping_ParAction (CircusAction ca)
	= undefined
mapping_ParAction (ParamActionDecl (x:xs) pa)
	= undefined
...
mapping_ProcessDef (ProcDefIndex (x:xs) pd)
	= undefined
mapping_ProcessDef (ProcDefSpot (x:xs) pd)
	= undefined

-- Based on D24.1 - COMPASS
