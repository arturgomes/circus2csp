...
zpara_string pi t = render (print_csp_para pi t)

zpred_string :: PrintInfo -> ZPred -> String
zpred_string pi t = render (print_csp_pred pi t)

zexpr_string :: PrintInfo -> ZExpr -> String
zexpr_string pi t = render (print_csp_expr pi t)

zsexpr_string :: PrintInfo -> ZSExpr -> String
zsexpr_string pi t = render (print_csp_sexpr pi t)

zgenfilt_string :: PrintInfo -> ZGenFilt -> String
zgenfilt_string pi t = render (print_csp_genfilt pi t)
...
zpara_stdout pi t = zterm_stdout pi print_csp_para t
zpred_stdout pi t = zterm_stdout pi print_csp_pred t
zexpr_stdout pi t = zterm_stdout pi print_csp_expr t
...
print_csp_para :: PrintInfo -> ZPara -> Doc
print_csp_para pi (ZGivenSetDecl gs)
  = nest 4 (brackets (print_csp_ident pi gs))
print_csp_para pi (ZSchemaDef n (ZSchema st))
  | reorder_stext pi  = print_csp_ebox pi name end st
  | otherwise         = name $$ nest 4 (print_csp_stext pi{preced=None} st) $$ end
  where
  name = print_csp_sname pi n
  end  = text "\n"
print_csp_para pi (ZSchemaDef n se)
  =  nest 4 (name <+> text "=" <+> print_csp_sexpr (reset_sexpr pi) se)
  where
  name = print_csp_sname pi n
print_csp_para pi (ZAbbreviation v e)
  = nest 4 (print_csp_var pi v <+> text "==" <+> print_csp_expr (reset_expr pi) e)
    
print_csp_para pi (ZFreeTypeDef v bl)
  = nest 4 (print_csp_ftype pi v bl)
print_csp_para pi (ZPredicate p)
  = nest 4 (print_csp_pred pi p)
print_csp_para pi (ZAxDef st) = print_csp_ebox pi open end st
  where
  open = text "\\begin{axdef}"
  end  = text "\\end{axdef}"
-- TODO Can axdefs be reordered?
-- TODO Gendefs
print_csp_para pi _  = text "Not Implemented (print_csp_para)"


print_csp_ebox :: PrintInfo -> Doc -> Doc -> [ZGenFilt] -> Doc
print_csp_ebox pi open end st
  ...
  preds             = print_csp_gensfilts pi (filter isCheck st)
  sep_decls         = map (nest 4) (sep_list (<+> text "\\\\") decls)
  decls             = print_csp_gensfilts pi (filter (not .isCheck) st)
...
print_csp_ident :: PrintInfo -> ZVar -> Doc
print_csp_ident pi = text . show_zvar


print_csp_var :: PrintInfo -> ZVar -> Doc
print_csp_var pi v
  | null op   = print_csp_ident pi v
  | otherwise = plevel pi Brac (print_csp_opname pi (head op))
  where
  op = all_by_lstr v


print_csp_opname :: PrintInfo -> OpInfo -> Doc
print_csp_opname pi oi
  ...
  |  otherwise               = text "Not Implemented (print_csp_opname)"
  where
  uscr = text "\\_"
  op   = print_csp_ident pi (lstr oi)


print_csp_declname :: PrintInfo -> ZVar -> Doc
print_csp_declname pi v
  | null op   = print_csp_ident pi v
  | otherwise = print_csp_opname pi (head op)
  where
  op = all_by_lstr v


print_csp_stext :: PrintInfo -> [ZGenFilt] -> Doc
print_csp_stext pi st
  | reorder_stext pi = fsep (sep_decls ++ opt_preds)
  | otherwise        = sep (sep_list (<> text ";") unord_st)
  where
  unord_st          = print_csp_gensfilts pi st
  opt_preds         = if null combined_preds
		      then []
		      else (text "|"):combined_preds
  combined_preds    = sep_list (<+> text " and") preds
  preds             = print_csp_gensfilts pi (filter isCheck st)
...
print_csp_gensfilts :: PrintInfo -> [ZGenFilt] -> [Doc]
print_csp_gensfilts pi l = map (print_csp_genfilt pi) l

print_csp_genfilt :: PrintInfo -> ZGenFilt -> Doc
print_csp_genfilt pi (Include se)
  = print_csp_sexpr (reset_sexpr pi) se
print_csp_genfilt pi (Choose v e)
  = print_csp_declname pi v <> text ":" <> print_csp_expr (reset_expr pi) e
print_csp_genfilt pi (Evaluate v e t)
  -- This is a Z extension that is introduced by module Optimize,
  -- so we make up some plausible syntax:   v==e:t
  = print_csp_declname pi v <> text "==" <> pexpr <> text ":" <> ptype
  where
  pexpr = (print_csp_expr (reset_expr pi) e)
  ptype = (print_csp_expr (reset_expr pi) t)
print_csp_genfilt pi (Check p)
  = print_csp_pred (reset_pred pi) p


print_csp_expr :: PrintInfo -> ZExpr -> Doc
-- print_csp_expr pi (ZUniverse)   = text "ZUniverse"
print_csp_expr pi (ZVar var)    = print_csp_var pi var
print_csp_expr pi (ZESchema se)
  = print_csp_sexpr pi{preced=Expr4ab} se
print_csp_expr pi (ZGiven gv)   = text (show gv)
print_csp_expr pi (ZGivenSet gsn) = print_csp_ident pi gsn
print_csp_expr pi (ZInt i)      = text (show i)
print_csp_expr pi e@(ZGenerator r v) =
  text "(_ " <> text(show r) <> text " " <> print_csp_expr pi v <> text ")"
print_csp_expr pi e@ZPowerSet{} = print_csp_unaryop pi (pregen_op e) (baseset e)
print_csp_expr pi e@ZFuncSet{}
  | is_sequence e &&
    domset e == ZIntSet (Just 1) Nothing &&
    not (null (fset_seq_op e))
    = print_csp_unaryop pi (head (fset_seq_op e)) (ranset e)
  | not (is_sequence e) &&
    not (null (fset_op e))
    = print_csp_binaryop pi (head (fset_op e)) (domset e) (ranset e)
  | otherwise
    = text ("Error: nonrepresentable function: " ++ show e ++ "  ")
print_csp_expr pi (ZCross el)   = plevel pi Expr1a pcrossl
  where
  pcrossl = fsep (sep_list (<+> text ".")
		  (map (print_csp_expr pi{preced=Expr1a}) el))
print_csp_expr pi (ZTuple el)
  = plevel pi Brac (fsep (comma_sep_exprs pi{preced=None} el))
-- always bracket
print_csp_expr pi (ZCall e1 e2) = print_csp_zcall pi e1 e2
print_csp_expr pi (ZSetDisplay el)
  = text "{" <> fsep (comma_sep_exprs pi{preced=None} el) <>  text "}"
print_csp_expr pi (ZSeqDisplay el)
  = text "<" <+> fsep (comma_sep_exprs pi{preced=None} el)
     <+>  text ">"
print_csp_expr pi (ZSetComp st opte)
  = text "{" <> print_csp_stext pi{preced=None} st
     <+> print_csp_optexpr (reset_expr pi) opte <> text "}"
print_csp_expr pi (ZLambda st e)
  = plevel pi Expr0 (text "\\" <+> print_csp_stext pi{preced=None} st
		      <+> text "@" <+> print_csp_expr (reset_expr pi) e)
print_csp_expr pi (ZMu st opte)
  = plevel pi Expr0 (text "\\mu" <+> print_csp_stext pi{preced=None} st
		      <+> print_csp_optexpr pi{preced=Expr0} opte)
print_csp_expr pi (ZELet ldl e)
  = plevel pi Expr0 (fsep [text "let", letdef,
			    text "within", print_csp_expr pi{preced=Expr0} e])
  where
  letdef = fsep(sep_list (<> text ";") (map (print_csp_letdef pi) ldl))
print_csp_expr pi (ZIf_Then_Else p e1 e2)
  = plevel pi Expr (fsep[(text "if" <+> print_csp_pred (reset_pred pi) p),
			  (text "then" <+> print_csp_expr (reset_expr pi) e1),
			  (text "else" <+> print_csp_expr (reset_expr pi) e2)])
print_csp_expr pi (ZSelect e v)
  = plevel pi Expr4 (print_csp_expr pi{preced=Expr4} e
		     <> text "." <> print_csp_var pi v)
print_csp_expr pi (ZTheta se)
  = plevel pi Expr4ab (text "\\theta" <+> print_csp_sexpr pi{preced=Expr4ab} se)
-- The remaining ones are mostly used for evaluation results
print_csp_expr pi (ZReln r)     = print_csp_var pi (lstr(reln_op r))
print_csp_expr pi (ZFunc1 uo)   = print_csp_var pi (lstr(unary_op uo))
print_csp_expr pi (ZFunc2 bo)   = print_csp_var pi (lstr(binary_op bo))
print_csp_expr pi (ZStrange ZIter)     = text "iter"  -- TODO: display as R^n ?
print_csp_expr pi (ZStrange ZDisjoint) = text "\\disjoint" -- TODO: format as a prefix relation?
print_csp_expr pi (ZFSet s) = print_csp_expr pi (ZSetDisplay (FiniteSets.set2list s))
print_csp_expr pi (ZIntSet low high) = print_csp_intset pi low high
-- print_csp_expr pi (ZBinding lvl)
--   = text "\\lblot" <+> bc <+> text "\\rblot"
--   where
--   bc = fsep (sep_list (<> text ",") (map (print_csp_bindmem pi) lvl))
print_csp_expr pi (ZFree0 v) = print_csp_var pi v
print_csp_expr pi (ZFree1 v e) = print_csp_expr pi (ZCall (ZVar v) e)
print_csp_expr pi (ZFreeType v bl) = print_csp_var pi v
print_csp_expr pi _      = text "Not Implemented (print_csp_expr)"


print_csp_bindmem :: PrintInfo -> (ZVar, ZValue) -> Doc
print_csp_bindmem pi (v,val)
  = print_csp_var pi v <> text "==" <> print_csp_expr pi{preced=None} val

print_csp_intset :: PrintInfo -> Maybe ZInt -> Maybe ZInt -> Doc
print_csp_intset pi Nothing   Nothing   = text "\\num"
print_csp_intset pi (Just 0)  Nothing   = text "\\nat"
print_csp_intset pi (Just 1)  Nothing   = text "\\nat_1"
print_csp_intset pi (Just lo) (Just hi)
  = plevel pi Op2 (fsep [zint lo,text "..",zint hi])
-- print_csp_intset pi Nothing (Just hi)
--   = text "\\{x:\\num | x" <+> text "<=" <+> zint hi <> text "\\}"
-- print_csp_intset pi (Just lo) Nothing
--   = text "\\{x:\\num | x" <+> text ">=" <+> zint lo <> text "\\}"

print_csp_letdef :: PrintInfo -> (ZVar,ZExpr) -> Doc
print_csp_letdef pi (v,e) = fsep [(print_csp_var pi v <+> text "=="), print_csp_expr (reset_expr pi) e]


print_csp_optexpr :: PrintInfo ->  (Maybe ZExpr) -> Doc
print_csp_optexpr pi Nothing  = empty
print_csp_optexpr pi (Just e) = text "@" <+> print_csp_expr pi e


comma_sep_exprs :: PrintInfo -> [ZExpr] -> [Doc]
comma_sep_exprs pi el
  = sep_list (<> text ",") (map (print_csp_expr (reset_expr pi)) el)


print_csp_zcall :: PrintInfo -> ZExpr -> ZExpr -> Doc
print_csp_zcall pi e@(ZVar v) args@(ZTuple[e1,e2])
  | null bop  = print_csp_zcall1 pi e args
  | otherwise = print_csp_binaryop pi (head bop) e1 e2
  where
  bop = bop_by_lstr v
print_csp_zcall pi (ZVar v) e = print_csp_unaryop pi (uop_by_lstr v) e
print_csp_zcall pi (ZCall (ZReln r) e1) e2 = error "ZCall ZReln e1 e2 found!" --print_csp_binaryop pi (reln_op r) e1 e2
print_csp_zcall pi (ZFunc1 uo) e = print_csp_unaryop pi (unary_op uo) e
print_csp_zcall pi (ZFunc2 ZFirst) (ZTuple[a,b]) = print_csp_unaryop pi (binary_op ZFirst) (ZTuple[a,b])
print_csp_zcall pi (ZFunc2 ZSecond) (ZTuple[a,b]) = print_csp_unaryop pi (binary_op ZSecond) (ZTuple[a,b])
print_csp_zcall pi (ZFunc2 bo) (ZTuple[a,b]) = print_csp_binaryop pi (binary_op bo) a b
print_csp_zcall pi e1 e2 = print_csp_zcall1 pi e1 e2

print_csp_zcall1 pi e e1    -- Function Arg list
  = plevel pi Expr3 fcall
  where
  fcall = fcat [print_csp_expr pi{preced=Expr3} e, space,
		print_csp_expr (incr_prec pi) e1]  -- TODO: check this precedence!
  space = text "~"


print_csp_unaryop :: PrintInfo -> OpInfo -> ZExpr -> Doc
print_csp_unaryop pi oi e
  ...
  where
  pexpr = print_csp_expr (update_prec pi oi) e
  pop   = print_csp_ident pi (lstr oi)
  oprec = prec oi

print_csp_binaryop :: PrintInfo -> OpInfo -> ZExpr -> ZExpr -> Doc
print_csp_binaryop pi oi e1 e2
  ...
  where
  oprec   = prec oi
  lexpr   = print_csp_expr (setl_prec pi oi) e1
  rexpr   = print_csp_expr (setr_prec pi oi) e2
  unambe1 = print_csp_expr (reset_expr pi) e1
  unambe2 = print_csp_expr (reset_expr pi) e2
  op      = print_csp_ident pi (lstr oi)
...
print_csp_pred :: PrintInfo -> ZPred -> Doc
print_csp_pred pi ZFalse{} = text "false"
-- print_csp_pred pi ZTrue{}  = text "true"
-- print_csp_pred pi (ZPre se)
--   = plevel pi Predu (text "\\pre" <+> print_csp_sexpr pi{preced=Predu} se)
print_csp_pred pi (ZAnd p1 p2)
  = plevel pi Pred1c (fsep [lpred, text "and", rpred])
  where
  lpred = print_csp_pred pi{preced=Pred1c} p1
  rpred = print_csp_pred pi{preced=Predu} p2
print_csp_pred pi (ZOr p1 p2)
  = plevel pi Pred1b (fsep [lpred, text "or", rpred])
  where
  lpred = print_csp_pred pi{preced=Pred1b} p1
  rpred = print_csp_pred pi{preced=Pred1c} p2
print_csp_pred pi (ZImplies p1 p2)
  = plevel pi Pred1a (fsep [ lpred, text "=>", rpred])
  where
  lpred = print_csp_pred pi{preced=Pred1b} p1
  rpred = print_csp_pred pi{preced=Pred1a} p2
print_csp_pred pi (ZIff p1 p2)
  = plevel pi Pred1 (fsep [lpred, text "<=>", rpred])
  where
  lpred = print_csp_pred pi{preced=Pred1} p1
  rpred = print_csp_pred pi{preced=Pred1a} p2
print_csp_pred pi (ZNot (ZMember e1 e2))
  = print_csp_inrel pi (ZTuple [e1,e2]) (ZReln ZNotin)
-- special case for \notin
print_csp_pred pi (ZNot (ZEqual e1 e2))
  = print_csp_inrel pi (ZTuple [e1,e2]) (ZReln ZNeq)
-- special case for \neq
print_csp_pred pi (ZNot p)
  = plevel pi Predu (text "not" <+> print_csp_pred pi{preced=Predu} p)
-- print_csp_pred pi (ZExists st p)
--   = plevel pi Pred (text "\\exists" <+> print_csp_pquantify pi st p)
-- print_csp_pred pi (ZExists_1 st p)
--   = plevel pi Pred (text "\\exists_1" <+> print_csp_pquantify pi st p)
-- print_csp_pred pi (ZForall st p)
--   = plevel pi Pred (text "\\forall" <+> print_csp_pquantify pi st p)
print_csp_pred pi (ZPLet ldl p)
  = plevel pi Pred (fsep [text "let",letdef,
			   text "within",print_csp_pred pi{preced=Expr0} p])
  where
  letdef = fsep(sep_list (<> text ";") (map (print_csp_letdef pi) ldl))
print_csp_pred pi (ZEqual e1 e2)
  = plevel pi Predu (fsep [(expr e1 <+> text "="), expr e2])
  where
  expr e = print_csp_expr pi{preced=Predu} e
print_csp_pred pi (ZMember e1 e2)
  = plevel pi Predu (print_csp_inrel pi{preced=Predu} e1 e2)
print_csp_pred pi (ZPSchema se)
  = plevel pi Predu (print_csp_sexpr pi{preced=Predu} se)
print_csp_pred pi _      = text "Not Implemented (print_csp_pred)"


print_csp_inrel :: PrintInfo -> ZExpr -> ZExpr -> Doc
print_csp_inrel pi (ZTuple [e1,e2]) (ZVar r)
  | (optype oi) == PreRel = fsep [rel, expr (ZTuple [e1,e2])]
  | otherwise             = fsep [expr e1, rel, expr e2] -- Infix
  where
  oi = reln_op_by_lstr r
  expr e = print_csp_expr pi{preced=Predu} e
  rel = print_csp_ident pi (lstr oi)
print_csp_inrel pi (ZTuple el) (ZVar r) -- el > 2
  | (optype oi) == PreRel = fsep [rel, print_csp_expr pi (ZTuple el)]
  | otherwise             = fsep [ text "member(",expr (ZTuple el), rel, text ")"]
  where
  oi = reln_op_by_lstr r
  expr e = print_csp_expr pi{preced=Predu} e
  rel = print_csp_ident pi r
print_csp_inrel pi (ZTuple el) (ZReln r)
  = fsep [expr e1, rel, expr e2]
  where
  expr e = print_csp_expr pi{preced=Predu} e
  rel = print_csp_ident pi (lstr(reln_op r))
  e1 = head el
  e2 = last el
print_csp_inrel pi e (ZVar r) -- el > 2
  | (optype oi) == PreRel = fsep [rel, print_csp_expr pi e]
  | otherwise             = fsep [ text "member(",expr e,rel, text ")"]
  where
  oi = reln_op_by_lstr r
  expr e = print_csp_expr pi{preced=Predu} e
  rel = print_csp_ident pi r
print_csp_inrel pi e1 e2 = fsep [ text "member(",expr e1, expr e2, text ")"]
  where
  expr e = print_csp_expr pi{preced=Predu} e
...
print_csp_pquantify :: PrintInfo -> [ZGenFilt] -> ZPred -> Doc
print_csp_pquantify pi st p
  = fsep [print_csp_stext newpi st, text "@", print_csp_pred newpi p]
  where
  newpi = pi{preced=None}


print_csp_sexpr :: PrintInfo -> ZSExpr -> Doc
-- print_csp_sexpr pi (ZSchema st)
--   = brackets (print_csp_stext pi{preced=SExpru} st)
-- print_csp_sexpr pi (ZSRef sn dl rl)
--   = plevel pi SRef (print_csp_sref pi{preced=SRef} sn dl rl)
-- print_csp_sexpr pi (ZS1 uo se)  = print_csp_s1op pi (s1_op uo) se
-- print_csp_sexpr pi (ZS2 bo se1 se2) = print_csp_s2op pi (s2_op bo) se1 se2
-- print_csp_sexpr pi (ZSHide se vl)
--   = plevel pi SExpr3 (sexpr <+> text "\\" <+> hide_labels)
--   where
--   hide_labels = parens (fsep (sep_list (<> text ",") (map (print_csp_declname pi) vl)))
--   sexpr = print_csp_sexpr pi{preced=SExpr3} se
-- print_csp_sexpr pi (ZSExists st se)
--   = plevel pi SExpr (text "\\exists" <+> print_csp_squantify pi st se)
-- print_csp_sexpr pi (ZSExists_1 st se)
--   = plevel pi SExpr (text "\\exists_1" <+> print_csp_squantify pi st se)
-- print_csp_sexpr pi (ZSForall st se)
--   = plevel pi SExpr (text "\\forall" <+> print_csp_squantify pi st se)
print_csp_sexpr pi _      = text "Not Implemented (print_csp_sexpr)"


-- print_csp_squantify :: PrintInfo -> [ZGenFilt] -> ZSExpr -> Doc
-- print_csp_squantify pi st se
--   = fsep [print_csp_stext newpi st, text "@", print_csp_sexpr newpi se]
--   where
...
print_csp_sref :: PrintInfo -> ZSName -> [ZDecor] -> [ZReplace] -> Doc
print_csp_sref pi n dl rl
  = print_csp_sname pi n <> decs <+> repls
  where
  decs     = text (concat dl)
  repls    = if null repllist
	     then empty
	     else brackets (fsep repllist)
  repllist = sep_list (<> text ",") (map (print_csp_replace pi) rl)


print_csp_replace :: PrintInfo -> ZReplace -> Doc
print_csp_replace pi (ZRename x y) = print_csp_declname pi y <> text "/" <> print_csp_declname pi x
print_csp_replace pi (ZAssign x y) = print_csp_declname pi x <> text ":=" <> print_csp_expr pi y


print_csp_sname :: PrintInfo -> ZSName -> Doc
print_csp_sname pi (ZSPlain s) = text s
-- print_csp_sname pi (ZSDelta s) = text "\\Delta" <+> text s
-- print_csp_sname pi (ZSXi s)    = text "\\Xi" <+> text s


print_csp_s1op :: PrintInfo -> OpInfo -> ZSExpr-> Doc
print_csp_s1op pi oi se = plevel pi oprec (op <+> rsexpr)
  where
  oprec = prec oi
  rsexpr = print_csp_sexpr pi{preced=SExpru} se
  op = print_csp_ident pi (lstr oi)


print_csp_s2op :: PrintInfo -> OpInfo -> ZSExpr -> ZSExpr -> Doc
print_csp_s2op pi oi se1 se2 = plevel pi oprec (fsep [lsexpr, op, rsexpr])
  where
  oprec = prec oi
  lsexpr = print_csp_sexpr (setl_prec pi oi) se1
  rsexpr = print_csp_sexpr (setr_prec pi oi) se2
  op = print_csp_ident pi (lstr oi)
-- Note all are infix operators


print_csp_ftype :: PrintInfo -> ZVar -> [ZBranch] -> Doc
print_csp_ftype pi v bl
  = print_csp_ident pi v <+> text "=" <+> branches
  where
  branches = fsep (sep_list sep (map (print_csp_branch pi) bl))
  sep = (<+> text "|")


print_csp_branch :: PrintInfo -> ZBranch -> Doc
print_csp_branch pi (ZBranch0 v) = print_csp_ident pi v
print_csp_branch pi (ZBranch1 v e)
  = print_csp_var pi v <> text "\\ldata" <+> print_csp_expr pi{preced=None} e
     <+> text "\\rdata"
