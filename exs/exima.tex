...
\end{schema}
\begin{zed} A653\_QueueingPortType \defs PortBaseType \land [MaxNbMessage: DecOrHexValueType]
\end{zed}
\begin{zed} A653\_SamplingPortType \defs PortBaseType
\end{zed}
        %</QueueingPortType>
...
%item Partition Status Type Definition
  ...
  %</PartitionVariableAttributes>
  %<*partitionstatus>
  \begin{zed} PARTITION\_STATUS\_TYPE \defs
    [Identifier:PARTITION\_ID\_TYPE;\\
        Period:SYSTEM\_TIME\_TYPE;\\
        Duration:SYSTEM\_TIME\_TYPE]\\
    \land PartitionVariables
  \end{zed}
  %</partitionstatus>
\end{enumerate}
...
\begin{circusaction}
  ...
  \t1  moduleEndInit \then\\
  \t1  ((\circmu~muX \circspot ExecPartitionServices\circseq muX)\\
...
\begin{circus}
  ...
  \circbegin~
\circstate PartitionHMSt \defs [pHM:A653\_PartitionHMTableType;\\
...
\begin{circus}
  ...
  \t2  \circspot
    \lpar \lchanset moduleInit, moduleEnd, moduleEndInit \rchanset \rpar pid:partitionIds  \circspot (A653\_Partition(pid, partHM(pid), sysError)) \lpar PartitionHMChannels \rpar PartitionHM(pid)\\
...
\begin{circusaction}
  ExecApex ~\circdef~ apex\_send\_mpHM\_table!mpHM!sysError \then\\
    \t1  moduleEndInit \then (\circmu~muX \circspot ExecApexServices \circseq muX)
...
\begin{circus}
  ...
  \t1 \circstate MultiPartitionHMSt \defs\\
...
%<*MajorTimeFrame>
  ...
  % \end{schema}
 \begin{circusaction} MajorTimeFrame \circdef \Skip \end{circusaction}   
%</MajorTimeFrame>
%<*NextPartition>
 \begin{circusaction} NextPartition \circdef \Skip \end{circusaction}   
...
%<*OSGetPartitionStatus>  
 \begin{circusaction} PartitionStatus \circdef \Skip \end{circusaction}   
...
%</OSGetPartitionStatus>
%<*SetPMode>
 \begin{circusaction} SetPMode \circdef \Skip \end{circusaction}   
...
\begin{circusaction}
  ...
  \t1 (
  \circmu~muX \circspot UpdateSystemTime \circseq \lschexpract NextPartition \rschexpract \circseq\\
  (\circif 
    (current\_partition.Offset >\\
    \t2  (system\_time \mod major\_time\_frame) ) \circthen\\
  \t1  \circwait (current\_partition.Offset -\\
  \t2 (system\_time \mod major\_time\_frame) ) \circseq muX\\
...
\begin{circus}
  ...
  \t1 \circstate\ TimerSt \defs [clock : SYSTEM\_TIME\_TYPE]
\end{circus}
%</TimerSt>
%<*Counter> 
\begin{circusaction}
  ...
  \circmu~muX~\circspot~\\
    ...
    \interleave~ (
        ...
        (\circmu~muY~\circspot~updateClock!clock \then muY) 
          %\Timeout{1} \Skip
       )
    )\circseq muX
   )
...
\begin{circus}
  ...
  \t1 \circstate\ ModuleHMSt \defs [modHM:\seq_1 A653\_ModuleHMTableType;\\
  \t4  sysError : \seq_1 A653\_ErrorIdentifierType]\\
