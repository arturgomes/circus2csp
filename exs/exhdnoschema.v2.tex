...
\circchannel getBicarbonateAcetate, setBicarbonateAcetate~:~CONCENTRATE\\% \comment{I don't see invariants for this one}\\
...
\circchannel getBloodLines, setBloodLines~:~BOOL\\
...
\begin{circusaction}
AutomatedSelfTest~\circdef~autSelfTest \then (setSignalLamp := GREEN)
\end{circusaction}
...
\begin{circusaction}
EnableUI~\circdef~(setSignalLamp := YELLOW)
...
\also ConnectPatientStartTherapy~\circdef~
  ...
  \\\t2 (setSignalLamp,hdMode := GREEN,RUN)
\end{circusaction}
...
\also InterruptDialysis~\circdef~ senHDMode!BYPASS \then (setSignalLamp := YELLOW)
\also CompleteTreatment~\circdef~ therapyEnding \then (setSignalLamp := YELLOW)
