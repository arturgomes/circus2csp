...
%\begin{axdef} TRADUTOR NAO SUPORTA NENHUMA DAS AXIOMATIC DEFINITIONS ABAIXO
%   n1, n2: \nat \\
%   x?, y!, z?: \nat
%\end{axdef}
...
\begin{circus}
  \circprocess\ DotTestSingle \circdef \circbegin \\
   ...
   %Test3 \circdef c.1!2?x \then d \then \Skip \\
   Test3 \circdef ddfse \then edfse \then \Skip \\
   %Test5 \circdef c?i!(f~i) \then \Skip \\
   %Test6 \circdef c.(S.y)?z \then \Skip \\ S.y é a chamada a uma variável de definição axiomática fora do formato suportado por JCircus
   %Test7 \circdef c.(x?)!(y!)!(z?) \then \Skip \\ Variaveis
   %Test8 \circdef c?x \prefixcolon (x > 1)!(f~x) \then \Skip \\ %SEM SUPORTE PARA F~X
   %Test9 \circdef c?x \then
   %               c?z \prefixcolon (z > x.1).(f~(x.2+x.3)) \then \Skip \\ %TRADUTOR NAO SUPORTA
   %Test10 \circdef g[\nat \cross \nat \cross \nat]?x!n1.n2 \then \Skip \\ %TRADUTOR NAO SUPORTA
   %Test10 \circdef g.n1.(f~n1) \then \Skip \\
   %\circspot \Skip \\ %VAMOS TORNAR O COMPORTAMENTO DESTE PROCESSO MAIS INTERESSANTE
	\circspot Test0 \circseq Test1 \circseq Test2 %\circseq Test3 
	\\
   \circend
\end{circus}
