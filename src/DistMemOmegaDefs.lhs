...
expand_action_names_CAction lst param (CSPParAction nm xp)
 ...
 | otherwise = (get_action1 nm lst xp lst)
...
rename_action (CActionCommand (CVarDecl zfs ma)) xfs
  = (replace_ParamAction_CAction1 ma (concat $ get_var_Choose zfs) xfs)
...
get_action1 _ _ _ [] = error ("Action not found")
get_action1 name lst param [(CParAction n (ParamActionDecl p (CircusAction a)))]
  | name == n = CSPUnParAction p a param
  | otherwise = error ("Action "++(name)++" not found")
get_action1 name lst param ((CParAction n (ParamActionDecl p (CircusAction a))):xs)
  | (name == n) = get_action1 name lst param [(CParAction n (ParamActionDecl p (CircusAction a)))]
  | otherwise = get_action1 name lst param xs
get_action1 name lst param (_:xs) = get_action1 name lst param xs
\end{code}
...
nmem_mkMemoryMerge bst =
  [CParAction "MemoryMerge" (CircusAction (CActionCommand (CVResDecl ((remdups $ filter_ZGenFilt_Choose bst)++[Choose ("ns",[],"") (ZCall (ZVar ("\\seq",[],"")) (ZVar ("NAME",[],"")))]) (nmem_mkMemoryMergeInternal bstTypes))))]
  where
    nbst = (remdups $ filter_ZGenFilt_Choose bst)
    bstTypes = ( get_binding_types nbst)
