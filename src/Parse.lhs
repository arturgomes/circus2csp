...
circus_proc_decl
  =	do {tok L_CIRCPROCESS; -- \circprocess N \circdef ProcessDef
    ...
    optnls;
      prc <- circus_proc_def;
    return (CParamProcess nm nm1 prc)}
...
circus_process_rename
  = do {nm <- circus_process_name;
      ...
      tok L_ASSIGN;
      zxprs <- opt [] ( circus_comm `sepby1` do {optnls; tok L_COMMA; optnls});;
      tok L_CLOSEBRACKET;
...
proc_par
  = do {cap <- circus_action_p `sepby1` many1 zsep; return (concat cap)}
--     +++
--     do {tok L_END_CIRCUS;
--       	cut;
...
--   = do {sc <- do {p <- many zschema_box;
--                   return ( map (\s -> ProcZPara s) (concat p))};
--         return sc}
--    +++ do {tok L_BEGIN_CIRCUSACTION;
--       	   cut;
--       	   cap1 <- many circus_action_p;
--       	   optnls;
...
circus_action_p
  ...
  +++ do  {name <- zword;
    ...
    cut;
    optnls;
    se <- do {tok L_OPENBRACKET;
              decls <- zdecl_part;
              optnls; tok L_VERT; cut; optnls;
              ax <- zaxiom_part ;
              tok L_CLOSEBRACKET;
...
circus_action_seq
  = chainl1 circus_action_rep (do {optnls;
      						tok L_CIRCSEQ;
      						optnls;
      						return CSPSeq})


circus_action_rep :: EParser ZToken CAction
circus_action_rep
  -- \Interleave Decl \circspot \linter NSExp \rinter Action
  =  do {tok L_REPINTERLEAVE;
      	optnls;
  ...
  +++ do {tok L_REPEXTCHOICE;
      ...
      optnls;
      	prcx<- circus_action;
      	return (CSPRepExtChoice (concat decls)  prcx)}
  -- \Semi x : T \circspot A
  +++ do {tok L_REPSEMI;
      optnls;
      decls <- zdecl_part;
      ...
      optnls;
      	prcx<- circus_action;
      return (CSPRepSeq (concat decls) prcx)}
  +++ circus_action_u
