...
\begin{code}
module DistMemMappingFunCircusCSP
(
  mapping_Circus,
  mapping_ZGenFilt_list,
  mkNewBindings
)
where
import AST
...
mapping_ZGenFilt_list spec (x:xs)
  = (mapping_ZGenFilt x) ++ "," ++ (mapping_ZGenFilt_list  spec xs)
\end{code}
\begin{code}
mapping_ZGenFilt :: ZGenFilt -> String
mapping_ZGenFilt  (Choose v _) = nfst v

mkNewBindings :: ZGenFilt -> ZGenFilt
mkNewBindings (Choose (a,b,c) t) = (Choose (a++"1",b,c) t)
\end{code}
...
mapping_CProc procn args spec (CProcRename n (zv:zvs) (xp:xps))
  = (mapping_CProc procn args spec n) ++"[["++ (mapping_rename procn spec (zv:zvs) (xp:xps)) ++"]]"
...
\end{code}
\begin{code}
mapping_CAction procn args spec (CSPUnParAction p a param)
  = "( let tmp("++(mapping_ZGenFilt_list spec p)++") = " ++ mapping_CAction procn args spec (a)
    ++ " within tmp("++  joinBy "," (map (mapping_ZExpr (get_delta_names1 spec)) param) ++"))"
\end{code}
\begin{circus}
...
\begin{code}
mapping_CAction procn args spec (CSPRepInterl [(Choose (x,[],tx) s)] a)
  = "( " ++ "||| "
    ++  x
    ++ " : "
    ++ (mapping_ZExpr (get_delta_names1 spec) s)
...
mapping_ZExpr lst (ZCall (ZVar ("+",[],[])) (ZTuple [n,m])) = "("++mapping_ZExpr lst (n) ++ " + " ++ mapping_ZExpr lst (m)++")"
mapping_ZExpr lst (ZCall (ZVar ("-",[],[])) (ZTuple [n,m])) = "("++mapping_ZExpr lst (n) ++ " - " ++ mapping_ZExpr lst (m)++")"
mapping_ZExpr lst (ZCall (ZVar ("\\035",[],[])) a) = "length(" ++ mapping_ZExpr lst (a)++")"
mapping_ZExpr lst (ZCall (ZVar ("\\\035",[],[])) a) = "length("++(mapping_ZExpr lst a)++")"
mapping_ZExpr lst (ZCall (ZVar ("\\hash",[],[])) a) = "length("++(mapping_ZExpr lst a)++")"
...
mapping_ZExpr lst (ZCall (ZVar ("\\iseq",[],[])) a) = "Seq("++(mapping_ZExpr lst a)++")"
