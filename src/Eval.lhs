...
module Eval
--
-- $Id: Eval.hs,v 1.77 2006-02-23 21:42:09 marku Exp $
--
...
zgenerate env ZPrefix s =
    do  seq <- coerceZSeq env s
	return (map ZSeqDisplay (Data.List.nub (Data.List.inits seq)))
zgenerate env ZSuffix s =
    do  seq <- coerceZSeq env s
	return (map ZSeqDisplay (Data.List.nub (Data.List.tails seq)))
zgenerate env ZInSeq s =
	...
	return (map ZSeqDisplay (Data.List.nub lists))
...
evalexpr (ZBinding pairs) env
  = do  let (names, zes) = unzip pairs
	...
	assertcheck (Data.List.sort names == names) "binding not sorted"
...
evalsetcomplazy (ZSetComp gfs (Just bs)) env =
    (maxDepth, map (Control.Monad.join . (evalexpr bs $?)) solns)
    where
...
applyfunc2 env ZExtract a b
 = do  -- this operator requires 'b' to be a sequence,
       ...
       seq2 <- Control.Monad.filterM (\(i,val) -> zmember env (ZInt i) a)
			     (zip [1..] seq)
...
applyfunc2 env ZFilter a b
 = do  -- this operator requires 'a' to be a sequence,
       ...
       seq2 <- Control.Monad.filterM (\val -> zmember env val b) seq
...
gen_and_filter2 depth (Choose v ze : gfs) env
  ...
  fset = Control.Monad.join (coerceZFSet env $? zeval)
...
evalpred (ZExists gfs zp) env
  ...
  = (exists . map (Control.Monad.join . (evalpred zp $?)) . gen_and_filter gfs) env
    where
...
evalpred (ZExists_1 gfs zp) env
  ...
  --
  = (exists0 . map (Control.Monad.join . (evalpred zp $?)) . gen_and_filter gfs) env
    where
...
evalpred (ZForall gfs zp) env
  ...
  = (forall0 . map (Control.Monad.join . (evalpred zp $?)) . gen_and_filter gfs) env
    where
...
-- true iff the argument is a Z base type.
is_base_type :: ZExpr -> Bool
is_base_type (ZCross es)
  = all is_base_type es
is_base_type (ZFreeType _ branches)
  = all is_base_type [t | (ZBranch1 _ t) <- branches]
is_base_type (ZIntSet Nothing Nothing)  -- \num
  = True
is_base_type (ZGivenSet _)
  = True
is_base_type ZUniverse
  = True
is_base_type s@ZPowerSet{}
  = is_non_empty s == False &&
    is_finite s == False &&
    is_base_type (baseset s)
is_base_type f@ZFuncSet{}
  = is_base_type (domset f) &&
    is_base_type (ranset f) &&
    is_function f == False &&
    is_total f == False &&
    is_onto f == False &&
    is_one2one f == False &&
    is_sequence f == False &&
    is_non_empty f == False &&
    is_finite f == False
...
zmember env _ ZUniverse
  = return True
zmember env _ basetype
  -- membership is always true when RHS is a base type,
  -- since we assume that the spec is well-typed.
...
zreln env ZPrefix a b =
    do  sa <- coerceZSeq env a
	sb <- coerceZSeq env b
	return (Data.List.isPrefixOf sa sb)
zreln env ZSuffix a b =
    do  sa <- coerceZSeq env a
	sb <- coerceZSeq env b
	return (Data.List.isSuffixOf sa sb)
zreln env ZInSeq a b =
    do  sa <- coerceZSeq env a
	sb <- coerceZSeq env b
	return (or [Data.List.isSuffixOf sa prefix | prefix <- Data.List.inits sb])
zreln env rel _ _
...
zintersect env ss =
	...
	others = (Data.List.\\) ss (ranges ++ fsets) in   -- ++ rels)
    zintersect2 env (filter (/= ZUniverse) (fset : range : Data.List.sort others))
--zintersect env ss =
