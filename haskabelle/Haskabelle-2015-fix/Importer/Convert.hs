...
newtype ContextM v = ContextM (ReaderT Customisations (StateT Context Gensym.GensymM) v)
    deriving (Applicative, Monad, MonadState Context, Functor, MonadReader Customisations)
...
newtype PatternM a = PatternM ((ReaderT PatternE (WriterT PatternO ContextM)) a)
    deriving (Applicative, Monad, MonadReader PatternE, MonadWriter PatternO, Functor)
...
newtype Conversion a = Conversion (ReaderT Config IO a)
    deriving (Applicative, Functor, Monad, MonadReader Config, MonadIO, MonadError IOError)
...
newtype GrovelM a = GrovelM (StateT GrovelS Conversion a)
    deriving (Applicative, Monad, Functor, MonadState GrovelS, MonadIO)
