...
module
  AsPatterns(Typerepa(..), Itself(..), Typerep(..), Nibble(..), Char(..),
              typerep_fun, Num(..), equal_num, Int(..), equal_int, plus_num,
              times_num, times_int, Times(..), one_int, One(..), uminus_int,
              bitM, dup, minus_int, sub, plus_int, less_num, less_eq_num,
              less_int, sgn_int, abs_int, apsnd, Ord(..), Plus(..), Numeral,
              numeral, Minus(..), Zero(..), Monoid_add, Semiring_1, Div(..),
              Semiring_numeral_div, divmod_step, divmod, less_eq_int,
              divmod_abs, divmod_int, div_int, mod_int, typerep_int, Term(..),
              Term_of(..), term_of_num, term_of_int, Natural(..),
              integer_of_natural, times_natural, less_eq_natural, max,
              minus_natural, plus_natural, one_natural, natural_of_integer,
              Nat(..), of_nat_aux, of_nat, sgn_integer, abs_integer,
              divmod_integer, plus_nat, one_nat, nat_of_integer, nat_of_natural,
              scomp, mod_integer, mod_natural, equal_natural, zero_natural,
              iterate, div_integer, div_natural, less_natural, minus_shift,
              next, log, range, random_int, Random(..), Narrowing_type(..),
              Narrowing_term(..), Partial_term_of(..), int_of_integer,
              partial_term_of_int, full_exhaustive_int, full_exhaustive_inta,
              Full_exhaustive(..), typerep_char, typerep_nibble, one_integer,
              Print(..), Narrowing_cons(..), Narrowing(..), MyRecord(..),
              MyRecord_pre_MyRecord, MyRecord_IITN_MyRecord, nth, foldr, pick,
              f, gen_length, size_list, natural_of_nat, select, bar, foo, one,
              two, nat_of_num, separate, print_list, long, quux, three, record,
              unsound, minus_nat, equal_nat, less_nat, less_eq_nat, divmod_nat,
              enum_char, mod_nat, char_of_nat, nat_of_char, collapse, valapp,
              listsum, select_weight, random_aux_list, nat_of_nibble,
              enum_nibble, nibble_of_nat, update_one, update_two, div_nat,
              case_char, update_three, sum, cons, times_nat, term_of_nibble,
              term_of_char, random_char, random_list, random_aux_MyRecord,
              non_empty, drawn_from, rec_MyRecord, around_zero, size_MyRecord,
              equal_nibble, equal_char, term_of_list, size_MyRecorda,
              equal_MyRecord, narrowing_nibble, random_MyRecord,
              term_of_MyRecord, typerep_MyRecord, full_exhaustive_list,
              partial_term_of_list, full_exhaustive_nibble,
              full_exhaustive_char, partial_term_of_nibble,
              partial_term_of_char, full_exhaustive_MyRecord,
              partial_term_of_MyRecord, typerep_MyRecord_pre_MyRecord,
...
of_nat n = of_nat_aux (\ i -> plus i onea) n zero;
...
instance Partial_term_of Int where {
  partial_term_of = partial_term_of_int;
};
...
instance Full_exhaustive Int where {
  full_exhaustive = full_exhaustive_inta;
};
...
long :: forall a. (Print a) => [a] -> [Char];
long (a5 : a6 : a7) =
  let {
    l = a5 : a6 : a7;
  } in print_list l ++
         [Char Nibble2 Nibble0, Char Nibble6 Nibble9, Char Nibble7 Nibble3,
           Char Nibble2 Nibble0, Char Nibble6 NibbleC, Char Nibble6 NibbleF,
           Char Nibble6 NibbleE, Char Nibble6 Nibble7, Char Nibble2 Nibble0,
           Char Nibble6 Nibble5, Char Nibble6 NibbleE, Char Nibble6 NibbleF,
           Char Nibble7 Nibble5, Char Nibble6 Nibble7, Char Nibble6 Nibble8,
           Char Nibble2 Nibble1];
long [] =
  (print_list :: [a] -> [Char]) [] ++
    [Char Nibble2 Nibble0, Char Nibble6 Nibble9, Char Nibble7 Nibble3,
      Char Nibble2 Nibble0, Char Nibble7 Nibble4, Char Nibble6 NibbleF,
      Char Nibble6 NibbleF, Char Nibble2 Nibble0, Char Nibble7 Nibble3,
      Char Nibble6 Nibble8, Char Nibble6 NibbleF, Char Nibble7 Nibble2,
      Char Nibble7 Nibble4, Char Nibble2 Nibble1];
long [v] =
  print_list [v] ++
    [Char Nibble2 Nibble0, Char Nibble6 Nibble9, Char Nibble7 Nibble3,
      Char Nibble2 Nibble0, Char Nibble7 Nibble4, Char Nibble6 NibbleF,
...
update_one :: [Int] -> MyRecord -> MyRecord;
update_one x (A uu f2 f3) = A x f2 f3;

update_two :: Int -> MyRecord -> MyRecord;
update_two x (A f1 uu f3) = A f1 x f3;
...
update_three :: Char -> MyRecord -> MyRecord;
update_three x (A f1 f2 uu) = A f1 f2 x;
                     ...
                     (\ a ->
                       (valapp
                          (valapp
                            (valapp
                              (A, (\ _ ->
                                    Const "AsPatterns.MyRecord.A"
                                      (Typerep "fun"
[Typerep "List.list" [Typerep "Int.int" []],
  Typerep "fun"
    [Typerep "Int.int" [],
      Typerep "fun"
        [Typerep "String.char" [], Typerep "AsPatterns.MyRecord" []]]])))
                              x)
                            y)
                          z,
                         a))))))])
...
rec_MyRecord :: forall a. ([Int] -> Int -> Char -> a) -> MyRecord -> a;
rec_MyRecord f (A x1 x2 x3) = f x1 x2 x3;
...
size_MyRecord (A x1 x2 x3) = Zero_nat;
...
size_MyRecorda (A x1 x2 x3) = Zero_nat;

equal_MyRecord :: MyRecord -> MyRecord -> Bool;
equal_MyRecord (A x1 x2 x3) (A y1 y2 y3) =
  x1 == y1 && equal_int x2 y2 && equal_char x3 y3;
...
term_of_MyRecord (A a b c) =
  App (App (App (Const "AsPatterns.MyRecord.A"
                  (Typerep "fun"
                    [Typerep "List.list" [Typerep "Int.int" []],
                      Typerep "fun"
                        [Typerep "Int.int" [],
                          Typerep "fun"
                            [Typerep "String.char" [],
                              Typerep "AsPatterns.MyRecord" []]]]))
             (term_of_list a))
        (term_of_int b))
    (term_of_char c);

typerep_MyRecord :: Itself MyRecord -> Typerepa;
typerep_MyRecord t = Typerep "AsPatterns.MyRecord" [];
