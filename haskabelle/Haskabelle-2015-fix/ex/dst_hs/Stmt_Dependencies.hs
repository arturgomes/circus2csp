...
module
  Stmt_Dependencies(Ord(..), Natural(..), integer_of_natural, plus_natural,
                     Plus(..), zero_natural, Zero(..), Monoid_add, Times(..),
                     Div(..), One(..), Numeral, Minus(..), Typerepa(..),
                     Term(..), Itself(..), Typerep(..), Term_of(..), Random(..),
                     Semiring_numeral_div, Narrowing_type(..),
                     Narrowing_term(..), Partial_term_of(..),
                     Full_exhaustive(..), Narrowing_cons(..), Narrowing(..),
                     Nat(..), Num(..), Set(..), Nibble(..), Char(..), Twin(..),
                     Twin_IITN_Twin, bitM, ball, foldr, less_eq_natural,
                     less_natural, one_natural, sgn_integer, abs_integer, apsnd,
                     divmod_integer, div_integer, div_natural, log, removeAll,
                     member, inserta, insert, times_natural, mod_integer,
                     mod_natural, max, natural_of_integer, minus_natural,
                     minus_shift, next, pick, scomp, equal_natural, iterate,
                     range, dest_Twin, h, g, f, k, listsum, select_weight,
                     valapp, bot_set, numeral, less_num, less_eq_num, cons,
                     plus_nat, plus_num, collapse, set2_Twin, set1_Twin,
                     pred_Twin, equal_num, times_num, non_empty, map_Twin,
                     rec_Twin, rel_Twin, size_Twin, random_aux_Twin,
                     one_integer, divmod_step, divmod, size_Twina, equal_Twin,
                     random_Twin, term_of_Twin, typerep_Twin,
                     full_exhaustive_Twin, partial_term_of_Twin,
                 ...
                 (\ a ->
                   (valapp
                      (valapp
                        (Twin,
                          (\ _ ->
                            Const "Stmt_Dependencies.Twin.Twin"
                              (Typerep "fun"
                                [(typerep :: Itself a -> Typerepa) Type,
                                  Typerep "fun"
                                    [(typerep :: Itself b -> Typerepa) Type,
                                      Typerep "Stmt_Dependencies.Twin"
[(typerep :: Itself a -> Typerepa) Type,
  (typerep :: Itself b -> Typerepa) Type]]])))
                        x)
                      y,
...
term_of_Twin (Twin a b) =
  App (App (Const "Stmt_Dependencies.Twin.Twin"
             (Typerep "fun"
               [(typerep :: Itself a -> Typerepa) Type,
                 Typerep "fun"
                   [(typerep :: Itself b -> Typerepa) Type,
...
typerep_Twin_IITN_Twin t =
  Typerep "Stmt_Dependencies.Twin.Twin_IITN_Twin"
    [(typerep :: Itself a -> Typerepa) Type,
      (typerep :: Itself b -> Typerepa) Type];
