...
module
  Tree(Ord(..), Natural(..), integer_of_natural, plus_natural, Plus(..),
        ...
        Tree(..), Nibble(..), Char(..), Tree_IITN_Tree, bitM, ball, fold,
        plus_nat, size, foldr, less_eq_natural, less_natural, one_natural,
        sgn_integer, abs_integer, apsnd, divmod_integer, div_integer,
        div_natural, log, removeAll, membera, inserta, insert, member,
        times_natural, mod_integer, mod_natural, max, natural_of_integer,
        minus_natural, minus_shift, next, pick, less_eq_nat, less_nat, insertb,
        scomp, equal_natural, iterate, range, sup_set, bot_set, set_Tree,
        pred_Tree, map_Tree, rec_Tree, rel_Tree, plus_nata, size_Tree, listsum,
        select_weight, collapse, valapp, random_aux_Tree, sum, numeral,
        less_num, less_eq_num, cons, plus_num, equal_num, times_num, size_Treea,
        non_empty, equal_Tree, random_Tree, term_of_Tree, typerep_Tree,
        one_integer, divmod_step, divmod, full_exhaustive_Tree,
        partial_term_of_Tree, typerep_Tree_IITN_Tree)
  where {
...
size :: forall a. Tree a -> Nata;
size (Tip a) = Suca Zero;
...
insertb :: forall a. a -> Tree a -> Tree a;
insertb x (Tip y) = Branch (Tip x) (Tip y);
insertb x (Branch y z) =
  (if less_eq_nat (size y) (size z) then Branch (insertb x y) z
    else Branch y (insertb x z));
...
set_Tree :: forall a. (Eq a) => Tree a -> Set a;
set_Tree (Tip x1) = insert x1 bot_set;
set_Tree (Branch x21 x22) = sup_set (set_Tree x21) (set_Tree x22);

pred_Tree :: forall a. (Eq a) => (a -> Bool) -> Tree a -> Bool;
pred_Tree p = (\ x -> ball (set_Tree x) p);

map_Tree :: forall a b. (a -> b) -> Tree a -> Tree b;
map_Tree f (Tip x1) = Tip (f x1);
map_Tree f (Branch x21 x22) = Branch (map_Tree f x21) (map_Tree f x22);

rec_Tree ::
  forall a b. (a -> b) -> (Tree a -> Tree a -> b -> b -> b) -> Tree a -> b;
rec_Tree f1 f2 (Tip x1) = f1 x1;
rec_Tree f1 f2 (Branch x21 x22) =
  f2 x21 x22 (rec_Tree f1 f2 x21) (rec_Tree f1 f2 x22);

rel_Tree :: forall a b. (a -> b -> Bool) -> Tree a -> Tree b -> Bool;
rel_Tree r (Tip x1) (Branch y21 y22) = False;
rel_Tree r (Branch y21 y22) (Tip x1) = False;
rel_Tree r (Tip x1) (Tip y1) = r x1 y1;
rel_Tree r (Branch x21 x22) (Branch y21 y22) =
  rel_Tree r x21 y21 && rel_Tree r x22 y22;

plus_nata :: Nat -> Nat -> Nat;
plus_nata (Suc m) n = plus_nata m (Suc n);
plus_nata Zero_nat n = n;

size_Tree :: forall a. (a -> Nat) -> Tree a -> Nat;
size_Tree x (Tip x1) = plus_nata (x x1) (Suc Zero_nat);
size_Tree x (Branch x21 x22) =
  plus_nata (plus_nata (size_Tree x x21) (size_Tree x x22)) (Suc Zero_nat);
...
random_aux_Tree i j s =
  collapse
    (select_weight
      [(i, scomp (random_aux_Tree (minus_natural i one_natural) j)
             (\ x ->
                                ...
                                (Typerep "fun"
                                  [Typerep "Tree.Tree"
                                    ...
                                    Typerep "fun"
                                      [Typerep "Tree.Tree"
 [(typerep :: Itself a -> Typerepa) Type],
Typerep "Tree.Tree" [(typerep :: Itself a -> Typerepa) Type]]])))
                          x)
                        y,
                       a))))),
        (one_natural,
...
size_Treea :: forall a. Tree a -> Nat;
size_Treea (Tip x1) = Suc Zero_nat;
size_Treea (Branch x21 x22) =
  plus_nata (plus_nata (size_Treea x21) (size_Treea x22)) (Suc Zero_nat);
...
equal_Tree (Tip x1) (Branch x21 x22) = False;
equal_Tree (Branch x21 x22) (Tip x1) = False;
equal_Tree (Branch x21 x22) (Branch y21 y22) =
  equal_Tree x21 y21 && equal_Tree x22 y22;
equal_Tree (Tip x1) (Tip y1) = x1 == y1;
...
term_of_Tree (Branch a b) =
  App (App (Const "Tree.Tree.Branch"
             (Typerep "fun"
               [Typerep "Tree.Tree" [(typerep :: Itself a -> Typerepa) Type],
                 Typerep "fun"
                   [Typerep "Tree.Tree"
...
typerep_Tree t = Typerep "Tree.Tree" [(typerep :: Itself a -> Typerepa) Type];
...
partial_term_of_Tree ty (Narrowing_constructor (1 :: Integer) [b, a]) =
  App (App (Const "Tree.Tree.Branch"
             (Typerep "fun"
               [Typerep "Tree.Tree" [(typerep :: Itself a -> Typerepa) Type],
                 Typerep "fun"
        ...
        ((partial_term_of_Tree :: Itself (Tree a) -> Narrowing_term -> Term)
          Type a))
    ((partial_term_of_Tree :: Itself (Tree a) -> Narrowing_term -> Term) Type
      b);
partial_term_of_Tree ty (Narrowing_constructor (0 :: Integer) [a]) =
  App (Const "Tree.Tree.Tip"
        (Typerep "fun"
          [(typerep :: Itself a -> Typerepa) Type,
            Typerep "Tree.Tree" [(typerep :: Itself a -> Typerepa) Type]]))
...
partial_term_of_Tree ty (Narrowing_variable p tt) =
  Free "_" (Typerep "Tree.Tree" [(typerep :: Itself a -> Typerepa) Type]);

typerep_Tree_IITN_Tree ::
  forall a. (Typerep a) => Itself (Tree_IITN_Tree a) -> Typerepa;
typerep_Tree_IITN_Tree t =
  Typerep "Tree.Tree.Tree_IITN_Tree" [(typerep :: Itself a -> Typerepa) Type];
