...
module
  Primitive(Ord(..), Natural(..), integer_of_natural, plus_natural, Plus(..),
             zero_natural, Zero(..), Monoid_add, Times(..), Div(..), One(..),
             Numeral, Minus(..), Semiring_numeral_div, Itself(..), Nat(..),
             Num(..), Nibble(..), Char(..), Nata(..), Typerep(..), Boolean(..),
             Term(..), Nat_IITN_Nat, Narrowing_type(..), Narrowing_term(..),
             Narrowing_cons(..), Boolean_IITN_Boolean, bitM, foldr,
             ...
             minus_natural, minus_shift, next, pick, ida, scomp, equal_natural,
             iterate, range, foo, decr, incr, isOdd, isEven, listsum,
             select_weight, rec_Nat, valapp, plus_nat, size_Nat, collapse,
             random_aux_Nat, sum, numeral, less_num, less_eq_num, cons,
             plus_num, equal_num, times_num, random_aux_Boolean, rec_Boolean,
             size_Boolean, non_empty, size_Nata, equal_Nat, random_Nat,
             term_of_Nat, typerep_Nat, size_Booleana, one_integer, divmod_step,
             divmod, equal_Boolean, random_Boolean, term_of_Boolean,
             typerep_Boolean, narrowing_Boolean, full_exhaustive_Nat,
             partial_term_of_Nat, typerep_Nat_IITN_Nat, full_exhaustive_Boolean,
             partial_term_of_Boolean, typerep_Boolean_IITN_Boolean)
  where {
...
isOdd (Succ (Succ va)) = isEven (decr (Succ (Succ va)));

isEven :: Nata -> Boolean;
isEven Zero = Verum;
isEven (Succ Zero) = Falsum;
isEven (Succ (Succ va)) = isOdd (decr (Succ (Succ va)));
...
rec_Nat f1 f2 (Succ x2) = f2 x2 (rec_Nat f1 f2 x2);
...
size_Nat (Succ x2) = plus_nat (size_Nat x2) (Suc Zero_nat);
             ...
             (\ x ->
               (\ a ->
                 (valapp
                    (Succ,
                      (\ _ ->
                        Const "Primitive.Nat.Succ"
                          (Typerep "fun"
                            [Typerep "Primitive.Nat" [],
                              Typerep "Primitive.Nat" []])))
                    x,
                   a)))),
        (one_natural,
          (\ a ->
            ((Zero,
               (\ _ ->
                 Const "Primitive.Nat.Zero" (Typerep "Primitive.Nat" []))),
              a)))])
    s;
...
random_aux_Boolean ::
  Natural ->
    Natural ->
      (Natural, Natural) -> ((Boolean, () -> Term), (Natural, Natural));
random_aux_Boolean i j s =
  collapse
    (select_weight
      [(one_natural,
         (\ a ->
           ((Verum,
              (\ _ ->
                Const "Primitive.Boolean.Verum"
                  (Typerep "Primitive.Boolean" []))),
             a))),
        (one_natural,
          (\ a ->
            ((Falsum,
               (\ _ ->
                 Const "Primitive.Boolean.Falsum"
                   (Typerep "Primitive.Boolean" []))),
              a)))])
    s;

rec_Boolean :: forall a. a -> a -> Boolean -> a;
rec_Boolean f1 f2 Verum = f1;
rec_Boolean f1 f2 Falsum = f2;

size_Boolean :: Boolean -> Nat;
size_Boolean Verum = Zero_nat;
size_Boolean Falsum = Zero_nat;
...
size_Nata (Succ x2) = plus_nat (size_Nata x2) (Suc Zero_nat);

equal_Nat :: Nata -> Nata -> Bool;
equal_Nat Zero (Succ x2) = False;
equal_Nat (Succ x2) Zero = False;
equal_Nat (Succ x2) (Succ y2) = equal_Nat x2 y2;
...
term_of_Nat (Succ a) =
  App (Const "Primitive.Nat.Succ"
        (Typerep "fun"
          [Typerep "Primitive.Nat" [], Typerep "Primitive.Nat" []]))
    (term_of_Nat a);
term_of_Nat Zero = Const "Primitive.Nat.Zero" (Typerep "Primitive.Nat" []);

typerep_Nat :: Itself Nata -> Typerep;
typerep_Nat t = Typerep "Primitive.Nat" [];

size_Booleana :: Boolean -> Nat;
size_Booleana Verum = Zero_nat;
size_Booleana Falsum = Zero_nat;
...
equal_Boolean :: Boolean -> Boolean -> Bool;
equal_Boolean Verum Falsum = False;
equal_Boolean Falsum Verum = False;
equal_Boolean Falsum Falsum = True;
equal_Boolean Verum Verum = True;

random_Boolean ::
  Natural -> (Natural, Natural) -> ((Boolean, () -> Term), (Natural, Natural));
random_Boolean i = random_aux_Boolean i i;

term_of_Boolean :: Boolean -> Term;
term_of_Boolean Falsum =
  Const "Primitive.Boolean.Falsum" (Typerep "Primitive.Boolean" []);
term_of_Boolean Verum =
  Const "Primitive.Boolean.Verum" (Typerep "Primitive.Boolean" []);

typerep_Boolean :: Itself Boolean -> Typerep;
typerep_Boolean t = Typerep "Primitive.Boolean" [];

narrowing_Boolean :: Integer -> Narrowing_cons Boolean;
narrowing_Boolean = sum (cons Verum) (cons Falsum);
...
partial_term_of_Nat ty (Narrowing_variable p tt) =
  Free "_" (Typerep "Primitive.Nat" []);

typerep_Nat_IITN_Nat :: Itself Nat_IITN_Nat -> Typerep;
typerep_Nat_IITN_Nat t = Typerep "Primitive.Nat.Nat_IITN_Nat" [];

full_exhaustive_Boolean ::
  ((Boolean, () -> Term) -> Maybe (Bool, [Term])) ->
    Natural -> Maybe (Bool, [Term]);
full_exhaustive_Boolean f i =
  (if less_natural zero_natural i
    then (case f (Verum,
                   (\ _ ->
                     Const "Primitive.Boolean.Verum"
...
partial_term_of_Boolean :: Itself Boolean -> Narrowing_term -> Term;
partial_term_of_Boolean ty (Narrowing_constructor (1 :: Integer) []) =
  Const "Primitive.Boolean.Falsum" (Typerep "Primitive.Boolean" []);
partial_term_of_Boolean ty (Narrowing_constructor (0 :: Integer) []) =
  Const "Primitive.Boolean.Verum" (Typerep "Primitive.Boolean" []);
partial_term_of_Boolean ty (Narrowing_variable p tt) =
  Free "_" (Typerep "Primitive.Boolean" []);

typerep_Boolean_IITN_Boolean :: Itself Boolean_IITN_Boolean -> Typerep;
typerep_Boolean_IITN_Boolean t =
  Typerep "Primitive.Boolean.Boolean_IITN_Boolean" [];
