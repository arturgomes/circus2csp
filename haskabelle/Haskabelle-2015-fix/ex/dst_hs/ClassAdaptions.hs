...
module
  ClassAdaptions(Ord(..), Natural(..), integer_of_natural, plus_natural,
                  Plus(..), zero_natural, Zero(..), Monoid_add, Times(..),
                  Div(..), One(..), Numeral, Minus(..), Eqa(..), Ident(..),
                  Semiring_numeral_div, Itself(..), Nat(..), Num(..),
                  Nibble(..), Char(..), Typerep(..), Nata(..), Term(..),
                  Nat_IITN_Nat, Narrowing_type(..), Narrowing_term(..),
                  Narrowing_cons(..), bitM, foldr, less_eq_natural,
                  ...
                  iterate, range, listsum, select_weight, fromEq, valapp,
                  fromIdent, sum, numeral, less_num, less_eq_num, cons, rec_Nat,
                  plus_nat, plus_num, collapse, size_Nat, equal_num, times_num,
                  random_aux_Nat, non_empty, eq_Nat, size_Nata, equal_Nat,
                  random_Nat, one_integer, divmod_step, divmod, term_of_Nat,
                  typerep_Nat, full_exhaustive_Nat, partial_term_of_Nat,
                  typerep_Nat_IITN_Nat)
  where {
...
fromEq :: forall a b. (Eqa a) => a -> a -> b -> Maybe b;
fromEq a1 a2 b = (if eq a1 a2 then Just b else Nothing);

valapp ::
  forall a b. (a -> b, () -> Term) -> (a, () -> Term) -> (b, () -> Term);
valapp (f, tf) (x, tx) = (f x, (\ _ -> App (tf ()) (tx ())));

fromIdent :: forall a b. (Ident a) => a -> a -> b -> Maybe b;
...
rec_Nat f1 f2 (Succ x1) = f1 x1 (rec_Nat f1 f2 x1);
...
size_Nat :: Nata -> Nat;
size_Nat (Succ x1) = plus_nat (size_Nat x1) (Suc Zero_nat);
             ...
             (\ x ->
               (\ a ->
                 (valapp
                    (Succ,
                      (\ _ ->
                        Const "ClassAdaptions.Nat.Succ"
                          (Typerep "fun"
                            [Typerep "ClassAdaptions.Nat" [],
                              Typerep "ClassAdaptions.Nat" []])))
                    x,
                   a)))),
        (one_natural,
...
eq_Nat (Succ m) (Succ n) = eq_Nat m n;
eq_Nat Zero (Succ n) = False;
eq_Nat (Succ m) Zero = False;

size_Nata :: Nata -> Nat;
size_Nata (Succ x1) = plus_nat (size_Nata x1) (Suc Zero_nat);
size_Nata Zero = Zero_nat;

equal_Nat :: Nata -> Nata -> Bool;
equal_Nat (Succ x1) Zero = False;
equal_Nat Zero (Succ x1) = False;
equal_Nat (Succ x1) (Succ y1) = equal_Nat x1 y1;
...
term_of_Nat Zero =
  Const "ClassAdaptions.Nat.Zero" (Typerep "ClassAdaptions.Nat" []);
term_of_Nat (Succ a) =
  App (Const "ClassAdaptions.Nat.Succ"
        (Typerep "fun"
          [Typerep "ClassAdaptions.Nat" [], Typerep "ClassAdaptions.Nat" []]))
    (term_of_Nat a);

typerep_Nat :: Itself Nata -> Typerep;
typerep_Nat t = Typerep "ClassAdaptions.Nat" [];
...
partial_term_of_Nat ty (Narrowing_variable p tt) =
  Free "_" (Typerep "ClassAdaptions.Nat" []);

typerep_Nat_IITN_Nat :: Itself Nat_IITN_Nat -> Typerep;
typerep_Nat_IITN_Nat t = Typerep "ClassAdaptions.Nat.Nat_IITN_Nat" [];
